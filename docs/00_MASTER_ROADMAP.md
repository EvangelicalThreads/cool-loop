...
| `07_COMPETITION_MAP.md` | Which version of the work goes where, and when. Read AFTER the research exists. |
...
- [ ] Compute thermal capacitance: C = M·cp. (2 kg oil × ~1,900 J/kg·K ≈ 3,800 J/K. With 4 kg oil: ~7,600 J/K.)
...
- [ ] Email OCSEF SRC (contact via ocsef.org) introducing the project and asking their preferred pre-approval route for summer-start projects. Save the reply.
...
- [ ] Create GitHub account; create private repo `cool-loop` with folders: `/hardware /src /data/raw /data/processed /figures /paper /safety /docs`.
...
- [ ] Tutorial 1: terminal basics + install Python on the laptop.
- [ ] Tutorial 2: Python crash course (variables, lists, dicts, functions, loops, reading/writing CSV) — 4–6 hrs, do all exercises.
- [ ] Run `src/thermal_sim.py` — a fake rig in software. Confirm it produces a CSV and a plot.
- [ ] Run `src/logger.py --sim` and watch it log the fake rig at 1 Hz.
...
- [ ] 🔴 First heat test (adult present, GFCI verified, heater plugged in LAST, watched 15 min, heater unplugged FIRST).
...
- [ ] MOSFET PWM board controlling fan speed from Pi GPIO18. Verify: 20% / 50% / 100% duty produces audibly/measurably different airflow.
...
- [ ] Workload emulation verified: heater PID setpoint can be stepped (e.g., 60W → 150W → 60W) to imitate bursty AI load. Script it via the PID controller's manual profile or timed manual changes logged in `heater_power` column.
...
- [ ] `pid_controller.py` holds T_fluid at setpoint (e.g., 38°C) by driving fan PWM. Tune conservatively (tutorial gives starting gains and a tuning recipe).
- [ ] 30-min run under a step workload: stable, no oscillation worse than ±1°C.

### 3.3 Reactive MPC (Software §9)
- [ ] Understand the MPC tutorial BEFORE running code. You must be able to explain to a judge, in one minute, what the optimizer minimizes and subject to what. The tutorial has that explanation; rehearse it.
- [ ] `mpc_controller.py` running: uses calibrated model, 20-min horizon, assumes future ambient = current ambient (that's what makes it "reactive").
...
- [ ] `forecast_mpc.py` running: same optimizer, but future ambient/workload trajectories come from `profiles.py` (the synthetic climate/workload generator).
- [ ] Banking logic verified in a scripted test: given a forecast heat spike at t+15 min, the controller measurably pre-cools below setpoint BEFORE the spike. Save this plot — it is Figure 2 of every paper and the poster centerpiece.
- [ ] 🔴 Fairness rule check: all three controllers use identical actuator limits, identical safety constraints, identical sensor inputs. Any asymmetry invalidates the comparison.
...
- [ ] Before each session: run the 10-point pre-flight in Protocol §2.
- [ ] After each trial: CSV auto-saved, notes field filled (anomalies, ambient conditions), photo if anything odd.
- [ ] Analyze AS YOU GO (`analyze.py` after every session). A broken trial found in week 9 costs one re-run; found in week 14 it wrecks the schedule.
- [ ] Weekly GitHub commit of raw data. Raw files are never edited — corrections happen in processing scripts, visibly.
...
**Goal: the paper, the repo, the reusable assets. Competitions come after — see `07_COMPETITION_MAP.md`.**
...
- [ ] GitHub repo public: cleaned code, README with reproduction steps, BOM with prices, safety doc, the V1 build manual → this is simultaneously the Rise service artifact.
- [ ] 3-minute video: rig tour + one live trial replay on the dashboard. Phone + tripod is fine.
- [ ] JEI or NHSJS submission package prepared (their format), submitted by ~Dec 1.
